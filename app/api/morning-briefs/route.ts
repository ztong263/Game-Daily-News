...
export async function GET(request:Request){try{localAccess(request);const date=z.iso.date().parse(new URL(request.url).searchParams.get("date"));const doc=await readVersions(date);
 return Response.json({versions:versionsOf(doc).map(b=>({id:b.id,version:b.version,date:b.date,title:b.title,sourceType:b.sourceType,ingestionChannel:b.ingestionChannel,createdAt:b.createdAt,active:doc.brief?.id===b.id&&doc.brief?.version===b.version}))},{headers:{"Cache-Control":"no-store"}});
