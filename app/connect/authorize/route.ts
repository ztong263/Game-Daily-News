...
export async function GET(request:Request){
 ...
 }catch(error){return new Response(consentFailure(error),{status:400,headers:{"Content-Type":"text/plain; charset=utf-8","Cache-Control":"no-store"}});}
}
export async function POST(request:Request){
 try{
  ...
  verifyConsent(request,origin,expected,actual);
  const {ownerId,db}=await authenticatedCloud();
  ...
  jar.delete({name:"publish-consent",path:"/connect/authorize"});
