...
test("budget confirmation only retries after explicit approval and remembers cancellation",async()=>{
 ...
 globalThis.fetch=async(url)=>{if(url==="/api/budget/approve"){grants++;return Response.json({ok:true});}requests++;return grants?Response.json({ok:true}):Response.json({error:"Confirm",budgetConfirmation:{operation:{id:"op",kind:"question"}}},{status:402});};
