...
async function researchLane(
  ...
  const research = await editorialResponse({
    ...
    // Each lane needs room for reasoning and a usable evidence report.
    max_output_tokens: budget.discoveryTokens,
    ...
    input:
      ...
      "合并相近主题检索，尽量留1–2次工具调用核对关键原文。搜索预算耗尽后如实报告证据不足，不补造。每条候选提供标题、来源名称、日期、URL和2–3句有实质事实的摘要。报告保持紧凑，不重复事实。\n" +
      "当地日期 " +
      date +
      "，时区 " +
      timezone +
      "。只搜索用户已启用的内容方向，普通新闻争取6–10条候选，游戏专项2–3条。如启用游戏雷达则核实游戏官方页面、开发者、发行/试玩状态，区分已验证信息与宣传。打开关键原文并比较。输出可核对材料、URL、发布日期、证据与局限。历史已选标题与来源：" +
      JSON.stringify(history),
  });
  await recordUsage(date,"discovery-"+lane,research);
  if(research.status!=="completed") throw Error(research.incomplete_details?.reason==="max_output_tokens"?"Research token limit":"Research response incomplete");
